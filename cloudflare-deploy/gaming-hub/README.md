...
### Stripe hosted checkout setup (recommended)

1. In Stripe Dashboard, create a Product named `Arcadia Grid Pro`.
2. Add a recurring monthly price of `1.99 USD`.
3. Create a Payment Link for that recurring price.
4. Copy that URL into `paymentLinks.monthlyProCheckoutUrl` in `assets/js/site-config.js`.
5. Create a second one-time Payment Link for donations that allows cards (and Apple Pay where supported).
6. Copy that URL into `paymentLinks.oneTimeCardCheckoutUrl`.
7. If Apple Pay should have a separate call-to-action, create another one-time Payment Link and set `paymentLinks.oneTimeApplePayUrl` to it.
8. In Stripe, open `Settings -> Payments -> Payment methods` and ensure Apple Pay is enabled.
9. For production, add and verify your domain in Stripe under `Settings -> Payment methods -> Apple Pay`.
10. Set `paymentLinks.oneTimePayPalUrl`, `paymentLinks.oneTimeCashAppUrl`, and `paymentLinks.zelleInstructions` with your real accounts.

Quick mapping reference:
- `monthlyProCheckoutUrl`: recurring $1.99/month subscription checkout link
- `oneTimeCardCheckoutUrl`: one-time card donation checkout link
- `oneTimeApplePayUrl`: Apple Pay-focused checkout link (can be same as Stripe card link)
- `oneTimePayPalUrl`: your PayPal.Me or PayPal checkout URL
- `oneTimeCashAppUrl`: your Cash App URL
- `zelleInstructions`: plain text instructions shown in the Zelle panel
